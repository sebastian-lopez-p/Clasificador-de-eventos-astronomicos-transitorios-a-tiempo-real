...
def write_to_csv(data_dict):
    ...
    try:
        with open(csv_filename, 'a', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            # Crear fila ordenada según los headers
            row = []
...
model = None
try:
    # Registrar la clase personalizada antes de cargar
...
def preprocess_metadata(metadata_row):
    ...
    try:
        ...
        # Escalar metadatos usando StandardScaler
        if not scaler_fitted:
            # Para el primer batch, usar los datos para fit_transform
            # En producción, deberías cargar un scaler pre-entrenado
...
def simulate_model_prediction(imagen, metadata):
    ...
    try:
        # Expandir dimensiones para simular batch
...
def process_batch_ml_enhanced(df, epoch_id):
    ...
    try:
        ...
        for idx, row in pdf.iterrows():
            ...
            # Agregar todos los metadatos al CSV
            for col_name in metadata_columns:
            ...
            if imagen_procesada is not None and metadata_procesados is not None:
                ...
                if prediccion is not None:
                    ...
                    # Agregar datos de predicción al CSV
                    csv_data.update({
